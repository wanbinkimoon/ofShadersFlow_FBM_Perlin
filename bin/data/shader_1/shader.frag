...
//  r.y = fbm(st + 1.0 * q + vec2(8.3,2.8) + 0.16 * u_time);
  r.x = fbm(st + 2.0 * q + vec2(1.7,9.2) + 0.15);
  r.y = fbm(st + 1.0 * q + vec2(8.3,2.8) + 0.16);
  
  vec2 s = vec2(0.);
  s.x = fbm(st + fbm(st + 3.2 * r + vec2(1.7, 4.2)));
  s.y = fbm(st + fbm(st + 2.1 * r + vec2(2.3, 2.8)));
  
  vec2 u = vec2(0.);
  u.x = fbm(st + fbm(st + 1.3 * s + vec2(4.7, 1.2)));
  u.y = fbm(st + fbm(st * s + vec2(2.3, 2.3)));
  
  vec2 v = vec2(0.);
  //  v.x = fbm(u + vec2(1.7 * u_time, 1.2));
  //  v.y = fbm(u + vec2(5.3 * u_time, 2.3));
  v.x = fbm(u + fbm(u + vec2(u_time, 1.2)));
  v.y = fbm(u + fbm(u + vec2(u_time, .4)));
  ...
  vec3 color = vec3(random(vec2(.1, .3)), random(vec2(.5, .8)), random(vec2(.3, .6)));
  ...
  color = mix(color,
              vec3(.2, sin(u_sound) + .12, .6),
              clamp(length(sin(q * u_sound)), .3, 1));
  ...
  vec3 color2 = vec3(random(vec2(.1, .4)), random(vec2(.2, .8)), random(vec2(.2, .4)));
  ...
  color2 = mix(color2,
               vec3(.1, .6, sin(u_sound * .8) + .2),
               clamp(length(sin(z.y *  f * u_time)), .2 , .8));
  
  color2 = mix(color2,
               vec3(cos(u_time * .1) - .1, cos(u_time * .1) - .3, cos(u_time * .3) - .2),
